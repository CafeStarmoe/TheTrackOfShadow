...
class LobbyPhase(override val game: Game) : PhaseAbstract() {
    // 剩余等待的时间
    private var waitTick = game.gameSetting.lobbyWaitTick
    ...
    override fun onTick() {
        ...
        if (game.playerModule.onlinePLayers.size < game.gameSetting.minPlayerCount) {
            // 重置剩余等待时间并提示玩家
    ...
    private fun notifyWaitMorePlayer() {
        game.playSound(game.playerModule.onlinePLayers, Sound.BLOCK_NOTE_BLOCK_HAT)
        game.sendTitle(
            game.playerModule.onlinePLayers,
    ...
    private fun notifyCountdown() {
        ...
        game.playSound(game.playerModule.onlinePLayers, Sound.BLOCK_NOTE_BLOCK_HAT)
        game.sendTitle(
            game.playerModule.onlinePLayers, "$waitTick".toGradientColor(countdownColor), "", 5, 20, 5
        )
        game.sendMsg(
            game.playerModule.onlinePLayers,
    ...
    private fun refreshBoard() {
        ...
        for (p in game.playerModule.onlinePLayers) {
            ...
            board.setSlot(
                8,
                "§f玩家: " + "${game.playerModule.onlinePLayers.size}/${game.gameSetting.maxPlayerCount}".toGradientColor(
                    listOf(
                        0xdcffcc,
                        0x9adbb1
                    )
            ...
            board.setSlot(
                6,
                if (game.playerModule.onlinePLayers.size >= game.gameSetting.minPlayerCount) "§f即将开始: " + "${waitTick}秒".toGradientColor(
